...
<nav
    class="flex shrink-0 border-[var(--color-violet-pale)] bg-[var(--color-surface)]
    ...
    {#each links as link}
        <a
            ...
            class="flex items-center gap-3 rounded-lg text-sm transition-colors
                ...
                ? 'bg-[var(--color-accent)] text-white max-md:bg-transparent max-md:text-[var(--color-accent)]'
                : 'text-[var(--color-text-muted)] hover:bg-[var(--color-violet-pale)]'}"
        >
