<script lang="ts">
    ...
    function computeRange(): { startDate: string; endDate: string } {
        ...
        if (selectedRange === "year") {
            // Only clamp to today when the selected year IS the current year —
            // otherwise (e.g. a past year) the range should run through
            // December 31st of that year, not today's date.
            const isCurrentYear = selectedYear === today.getFullYear();
            return {
                startDate: `${selectedYear}-01-01`,
                endDate: isCurrentYear ? todayStr : `${selectedYear}-12-31`,
            };
        }
        return { startDate: earliestDate ?? todayStr, endDate: todayStr };
    }
