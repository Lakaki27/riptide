...
onMount(async () => {
    const { authEnabled } = await apiFetch<{ authEnabled: boolean }>("/config", { skipAuth: true });
    ...
    if (auth.accessToken) {
        try {
            const me = await apiFetch<{ theme: string; language: string }>("/auth/me");
...
{#if ready}
    ...
    {:else}
        <div
            class="flex h-screen flex-col text-(--color-text-primary) bg-(--color-bg)"
        >
